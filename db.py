...
CREATE TABLE IF NOT EXISTS cabinets (
  ...
  async def get_user_cabinets(self, user_id: int) -> list[aiosqlite.Row] | None:
    try:
  ...
  async def switch_cabinet(self, user_id: int, id: int) -> bool:
    try:
  ...
  async def get_user_seller_name(self, token: str) -> str | None:
    try:
      async with self.db.execute("SELECT seller_name FROM cabinets WHERE token = ?", (token,)) as cursor:
        row = await cursor.fetchone()
        seller_name = row["seller_name"] if row else None
    except (aiosqlite.Error, TypeError):
  ...
  async def add_cabinet(self, user_id: int, token: str, seller_name: str, brand_name: str) -> bool | str:
    try:
  ...
  async def reset_token(self, user_id: int, token: str) -> bool:
    try:
